...
RSpec.describe "annual_leave_requests/new", type: :view do
  let(:user) { create(:user) }

  context "when there are errors present" do
    it "renders the error summary" do
      invalid_leave_request = build(:annual_leave_request, user_id: user.id)
      invalid_leave_request.errors.add(:date_from, "error 1")
      invalid_leave_request.errors.add(:date_from, "error 2")

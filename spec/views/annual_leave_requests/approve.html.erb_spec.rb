...
RSpec.describe "annual_leave_requests/approve", type: :view do
  ...
  context "when there are errors present" do
    it "renders the error summary" do
      ...
      invalid_leave_request.errors.add(:confirm_approval, "error 1")
      ...
      expect(rendered).to have_css("p.govuk-error-message", text: "Confirm approval error 1")
    end
  end
  ...
  context "when there are no errors present" do
    it "doesn't render the error summary" do
      ...
      expect(rendered).not_to have_content("Fix the following issue(s) and re-submit your response")
    end
  end
end
